...
export default function ManualEntry({ _onBack, token }) {
  ...
  const configuredFields = useMemo(() => {
    const fields = configuredComponents
      .map((item) => {
        const explicit = (item.fieldKey || "").trim();
        const fallback = COMPONENT_ALIAS_TO_FIELD[normalizeText(item.component)];
        const key = explicit || fallback || null;
        return {
          component: item.component,
          key,
          category: key ? FIELD_META[key]?.category || item.category : item.category,
  ...
  const findComponentMetaForPfLink = (linkedName) => {
    ...
    const linkedFieldKey = COMPONENT_ALIAS_TO_FIELD[normalized] || null;
  ...
  const resolveComponentAmount = (component, valuesMap, data, visited = new Set()) => {
    ...
    const entry = valuesMap[component.id] || { valueType: "amount", value: "" };
    if (
  ...
  const fetchPayslipConfig = async () => {
    ...
    try {
      ...
      (result.components || []).forEach((item) => {
        const fieldKey =
          (item.fieldKey || "").trim() ||
          COMPONENT_ALIAS_TO_FIELD[normalizeText(item.component)] ||
          normalizeText(item.component);
        nextValues[fieldKey] = {
      ...
      setFormData((prev) => {
        ...
        Object.entries(nextValues).forEach(([fieldKey, entry]) => {
          if (FIELD_META[fieldKey]) {
            next[fieldKey] = entry.value === "" ? "" : Number(entry.value);
          }
        });
        return next;
      });
  ...
  const handleSalaryComponentChange = (componentId, key, value) => {
    setSalaryComponentValues((prev) => {
      ...
      if (meta?.fieldKey && (key === "value" || key === "valueType")) {
        const computedAmount = resolveComponentAmount(meta, next, formData);
        setFormData((current) => {
          const updated = { ...current, [meta.fieldKey]: computedAmount };
          calculateNetSalary(updated, next);
          return updated;
        });
      }
      return next;
  ...
  const calculateNetSalary = (data, valuesMap = salaryComponentValues) => {
    ...
    mappedComponents.forEach((component) => {
      ...
      const category =
        (component.fieldKey && FIELD_META[component.fieldKey]?.category) ||
        component.category ||
        "earnings";
    ...
    const totalEarnings = round2(
      grossAmount > 0 ? grossAmount + additiveEarnings : fallbackEarnings
    );
  ...
  useEffect(() => {
    calculateNetSalary(formData, salaryComponentValues);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const validateForm = () => {
    ...
    if (
      visibleKnownFieldSet.has("basicSalary") &&
      (!formData.basicSalary || Number(formData.basicSalary) <= 0)
    )
  ...
  const handleSavePayroll = async () => {
    ...
    const payload = {
      ...
      medicalInsurance: Number(formData.medicalInsurance) || 0,
...
//         `${API_BASE_URL}/api/payroll/manual-entry`,
//         {
//           method: "POST",
//           headers: {
//             "Content-Type": "application/json",
