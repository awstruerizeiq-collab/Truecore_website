...
const getPayslipData = (payroll, company) => {
  ...
  const joiningDate = payroll?.joiningDate
    ? new Date(payroll.joiningDate).toLocaleDateString("en-IN", {
        day: "2-digit",
        month: "short",
        year: "numeric",
      })
  ...
  const totalEarnings = Number(
    payroll?.totalEarnings ?? earnings.reduce((sum, item) => sum + item.value, 0)
  );
  const totalDeductions = Number(
    payroll?.totalDeductions ?? deductions.reduce((sum, item) => sum + item.value, 0)
  );
...
const drawCommonPayslipBody = (doc, data, styles, startY) => {
  ...
  doc.text("This is a system-generated document.", pageWidth / 2, footerY + 18, {
    align: "center",
  });
};
...
const buildDoc = async (payroll, companyRaw, templateVariant) => {
  const doc = new jsPDF("p", "pt", "a4");
  const company = normalizeCompany(companyRaw);
  const data = getPayslipData(payroll, company);
  const logoData = await fetchImageAsDataUrl(company.logoUrl);
  const variant = String(templateVariant || "template_1").toLowerCase();

  if (variant === "template_2") renderTemplate2(doc, data, logoData);
  else if (variant === "template_3") renderTemplate3(doc, data, logoData);
  else renderTemplate1(doc, data, logoData);

  return { doc, data };
};

export const downloadPayslipWithTemplate = async ({
  payroll,
  company,
  templateVariant,
  fileName,
}) => {
  const { doc, data } = await buildDoc(payroll, company, templateVariant);
  const safeName =
    fileName ||
    `Payslip_${String(data.employee.name || "employee").replace(/\s+/g, "_")}_${String(
      data.employee.month || ""
    ).replace(/\s+/g, "_")}.pdf`;
  doc.save(safeName);
};

export const previewPayslipWithTemplate = async ({ payroll, company, templateVariant }) => {
  const { doc } = await buildDoc(payroll, company, templateVariant);
